...
final class ActToolsTests: XCTestCase {
    ...
    func testNamesAndRequiredArgs() {
        ...
        XCTAssertEqual(PerformActionTool(session: session).name, "perform")
        XCTAssertEqual(SetValueTool(session: session).name, "set_value")
        XCTAssertEqual(SetFocusTool(session: session).name, "set_focus")
        XCTAssertEqual(RevealTool(session: session).name, "reveal")

        let performSchema = PerformActionTool(session: session).descriptor["inputSchema"] as? [String: Any]
        XCTAssertEqual(performSchema?["required"] as? [String], ["ref", "action"])
    ...
    func testAllGateOnPermission() {
        let session = AXSession()
        XCTAssertTrue(PerformActionTool(session: session, isTrusted: notTrusted).call(["ref": "e1", "action": "AXPress"]).contains("accessibility_not_granted"))
        XCTAssertTrue(SetValueTool(session: session, isTrusted: notTrusted).call(["ref": "e1", "value": "x"]).contains("accessibility_not_granted"))
        XCTAssertTrue(SetFocusTool(session: session, isTrusted: notTrusted).call(["ref": "e1"]).contains("accessibility_not_granted"))
    ...
    func testUnknownRefIsStaleNotFired() {
        ...
        XCTAssertTrue(PerformActionTool(session: session, isTrusted: trusted).call(["ref": "never", "action": "AXPress"]).contains("stale_ref"))
        XCTAssertTrue(SetValueTool(session: session, isTrusted: trusted).call(["ref": "never", "value": "x"]).contains("stale_ref"))
        XCTAssertTrue(SetFocusTool(session: session, isTrusted: trusted).call(["ref": "never"]).contains("stale_ref"))
        XCTAssertTrue(RevealTool(session: session, isTrusted: trusted).call(["ref": "never"]).contains("stale_ref"))
    }

    func testMissingArgs() {
        let session = AXSession()
        XCTAssertTrue(PerformActionTool(session: session, isTrusted: trusted).call(["ref": "e1"]).contains("missing_ref_or_action"))
        XCTAssertTrue(SetValueTool(session: session, isTrusted: trusted).call(["ref": "e1"]).contains("missing_ref_or_value"))
        XCTAssertTrue(SetFocusTool(session: session, isTrusted: trusted).call([:]).contains("missing_ref"))
    }
    ...
    func testActToolsAdvertiseObserveSettle() {
        ...
        XCTAssertEqual(observeEnum(PerformActionTool(session: session).descriptor), ["none", "settle"])
        XCTAssertEqual(observeEnum(SetValueTool(session: session).descriptor), ["none", "settle"])
        XCTAssertEqual(observeEnum(SetFocusTool(session: session).descriptor), ["none", "settle"])
    ...
    func testObserveSettleStillHonorsStaleRefGuard() {
        ...
        XCTAssertTrue(PerformActionTool(session: session, isTrusted: trusted).call(["ref": "never", "action": "AXPress", "observe": "settle"]).contains("stale_ref"))
        XCTAssertTrue(SetValueTool(session: session, isTrusted: trusted).call(["ref": "never", "value": "x", "observe": "settle"]).contains("stale_ref"))
        XCTAssertTrue(SetFocusTool(session: session, isTrusted: trusted).call(["ref": "never", "observe": "settle"]).contains("stale_ref"))
